...
■特徴
------------------------------------------------------------------------------
在制作插件的时候，参考了por Masket的MBS MbileDirPad.js。

○本插件的特征
・为了在游戏画面外（黑带部分）设置按钮，不容易干扰游戏画面
・或按钮可以单独显示/不显示
・按钮的标准点画面四角的任何可以指定，纵有操作对应可能
・重视方向盘的操作性，可以调整触摸判定区域、斜方向的灵敏度等
　（詳細は下記の説明を参照）
・使用方向盘的移动和根据默认的目的地触摸而使用的移动
・特定的触摸操作，通过手势的按钮操作的扩展
...
@desc 决定按钮的位置。用从画面端的间隙的大小来指定。
 (右からの幅; 下からの幅) 初期値:10; 90
@default 10; 90

@param ActionBtn Orientation
@desc 想将决定按钮的基准位置改为右下以外的情况。
...
@desc 取消按钮的位置。用从画面端的间隙的大小来指定。
 (右からの幅; 下からの幅) 初期値:110; 10
@default 110; 10

@param CancelBtn Orientation
@desc 想把取消按钮的基准位置改为右下以外的情况。
...
@desc 在画面上左右转动的话，会成为PageUp / PageDon操作。
ステータス画面でキャラを切り替えたい時などに。初期値:true
@default true

@param HoldCanvas ActionBtn
@desc 长按下画面的话，就变成了按下决定按钮的状态。
初期値:true
@default true

@param OutCanvas CancelBtn
@desc 游戏画面外的黑带部分将被当作取消按钮。
初期値:false
@default false

@param OutCanvas ActionBtn
@desc 游戏画面以外的黑带部分，成为决定按钮。
初期値:false
@default false
*/
